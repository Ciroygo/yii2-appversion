...
class App extends ActiveRecord
{
    ...
    public function scopeIpStatus($appId)
    {
        ...
        if (!empty($ips)) {
            ...
            foreach ($ips as $ip) {
                $user_ip_arr = explode('.', $userIp);
                $ip_arr = explode('.', $ip);

                // 判断规则是否有 * ，有的话，替换到用户 ip 里面
                if (!empty($fl_any_arr = preg_grep("/\*/", $ip_arr))) {
                    $user_ip_arr = $fl_any_arr + $user_ip_arr;
                    ksort($user_ip_arr);
                }

                // 判断规则是否有 XX-XX，有的话替换到用户 ip 里面
                if (!empty($fl_scope_arr = preg_grep("/((2(5[0-5]|[0-4]\d))|[0-1]?\d{1,2}|\*)(-((2(5[0-5]|[0-4]\d))|[0-1]?\d{1,2}|\*))/", $ip_arr))) {
                    // 计算ip范围值
                    foreach ($fl_scope_arr as $key => $fl_scope) {
                        $fl_scope = explode('-', $fl_scope);
                        if ($fl_scope[0] <= $fl_scope[1]) {
                            // 如：100-200、 100-100 小于或相等的情况
                            if ($fl_scope[0] <= $user_ip_arr[$key] && $user_ip_arr[$key] <= $fl_scope[1]) {
                                $user_ip_arr[$key] = $fl_scope_arr[$key];
                            }
                        } else {
                            //如：155-100  取值范围 155 - 255   0-100
                            if (($fl_scope[0] <= $user_ip_arr[$key] && $user_ip_arr[$key] < 256) || (0 <= $user_ip_arr[$key] && $user_ip_arr[$key] <= $fl_scope[1])) {
                                $user_ip_arr[$key] = $fl_scope_arr[$key];
                            }
                        }
                    }
